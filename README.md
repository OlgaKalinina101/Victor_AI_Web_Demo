...
### android app: https://github.com/OlgaKalinina101/victor_ai_android (пока закрыт и будет открыт по факту доработки документации до 31.01.2025 г).
...
А для разработчиков здесь **точно нет ничего важного**. Но можете попробовать посмотреть здесь: https://github.com/OlgaKalinina101/victor_ai_backend

- Если вам сразу нужна установка — начинайте с:  
  👉 https://github.com/OlgaKalinina101/victor_ai_backend/blob/master/docs/install_guide.md  
  Он написан **для пользователя**, пошагово.

- Если хотите более мягкий и подробный ввод — он в репозитории backend:  
  👉 https://github.com/OlgaKalinina101/victor_ai_backend  

---

Этот репозиторий — **демонстрационный веб-интерфейс (frontend)**.  
Бэкенд (FastAPI) обычно живёт **в другом проекте / в другом окне PyCharm** — это нормально.
...
В проекте бэкенда (в соседнем окне) выполните:
...
## Как это всё связано (очень простое объяснение)
...
### В типичном случае: НИЧЕГО НЕ НАДО ВПИСЫВАТЬ

* В dev-режиме frontend **по умолчанию** пытается подключиться к API по адресу:
  `http://127.0.0.1:8000`
